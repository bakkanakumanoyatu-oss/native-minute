...
G5B Mobile Settings / Legal / Account Deletion Entry は `P1_REMEDIATED_PENDING_FINAL_READ_ONLY_REAUDIT` です。MobileはSettingsからVoice Setup、G5A canonical consent status、Privacy / Terms / Support、専用account-deletion entryへ到達できます。legal pageはofficial Capacitor Browserでconfigured canonical BFF originの固定allowlistだけを開き、URLへBearer/cookie/token/secretを渡しません。Mobile deletionはWeb cookie handoffなしのBearer-only adapterで既存domain serviceを再利用し、`cancelled` / `expired`を安全にdecodeして再申請可能なterminal stateとして扱います。POSTはbody/schema validationより先にBearer認証とuser導出を行います。actual deletion、confirmation、provider/Storage/DB/Auth cleanup、voice-only deletionは未実装です。P0=0、P1=0 at remediation validation、P2=1（approved Staging target fingerprint未提供のためexact Staging native Browser/build behavior未実施）。Gate 5全体はOPENのままです。詳細は [G5B result](./docs/g5b-mobile-settings-legal-and-deletion-entry-result.md) を参照してください。
