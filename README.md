...
- `listen` で見本音声を生成するとき、`natural / expressive / clear / slow` の voice style preset を選べる。ElevenLabs では preset を `voice_settings` に変換し、それ以外の provider では安全に無視する
- `listen` の見本音声と `review` の保存済み録音は、client-side の再生速度だけを `0.75x / 0.85x / 1.0x / 1.15x` で切り替えられる。provider 再生成や cache 変更はしない
...
- `progress` で最新結果、ベスト結果、改善の流れを見て、次に戻る 1 本を決める
...
7. `/progress` で最新結果とベスト結果の差を見て、次の 1 本に戻る

## guidance の読み方

主要画面は、次の役割語彙でそろえています。
...
- `Next action`
  - いま押す判断
- `Next decision`
  - listen で quality concern を開いたときの 3 択
- `Other actions`
  - 補助導線

listen では、この共通文法に加えて `音声状態` と `更新の意味` を分けて表示し、見本確認と更新判断が混ざりにくいようにしています。
不足前提がある画面では、`Recovery plan` が「何が足りないか」、`Next action` が「次に押す操作」、`Other actions` が「補助導線」を表します。

説明を増やすより、`次に何をすべきか` が迷いにくいことを優先しています。
...
5. `/scripts/[id]/listen` で見本音声を生成し、`音声状態` と `更新の意味`、`Next action` を確認する
6. voice や provider の前提が不足しているときは、その場の `Recovery plan / Next action` から `voice 設定` や `scripts` に戻る
7. `/scripts/[id]/record` で録音を選び、必要なら補助 transcript を入れて `評価して保存する`
8. `/scripts/[id]/review/[takeId]` で保存済み結果と保存済み録音、`Next action` に加えて `最新 / ベスト` の位置づけを確認する
9. `/progress` で最新結果とベスト結果の差、`Current step / Next step / Next action` を確認し、次に戻る 1 本を決める
