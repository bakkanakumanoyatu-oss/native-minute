...
G5B Mobile Settings / Legal / Account Deletion Entry は `IMPLEMENTED_VALIDATED` です。MobileはSettingsからVoice Setup、G5A canonical consent status、Privacy / Terms / Support、専用account-deletion entryへ到達できます。legal pageはofficial Capacitor Browserでconfigured canonical BFF originの固定allowlistだけを開き、URLへBearer/cookie/token/secretを渡しません。account-deletion request/statusはWeb cookie handoffなしのMobile Bearer adapterで既存domain serviceを再利用し、safe request stateだけを返します。actual deletion、confirmation、provider/Storage/DB/Auth cleanup、voice-only deletionは未実装です。P0=0、P1=0、P2=1（approved Staging target fingerprint未提供のためexact Staging native build/sync未実行）。Gate 5全体はOPENのままです。詳細は [G5B result](./docs/g5b-mobile-settings-legal-and-deletion-entry-result.md) を参照してください。
