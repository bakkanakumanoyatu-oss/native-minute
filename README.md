...
Mobile Personal Space P3は `HUMAN APPROVED / CLOSED` です。0031 Staging適用・限定BFF配備・iPhone更新install後、HumanがA〜Kの実機確認を完了しました。保存済み本人TakeをReviewで1タップ読込後再生し、独立した共有ボタンからiOS共有できます。My Takesから同じReviewを開けます。Bearer専用 `GET /api/mobile/takes/[takeId]/audio` はprivate/no-storeで音声本体を返し、public Storage URLは作りません。承認・検証範囲と残るrelease条件は [P3 checkpoint](./docs/qss-personal-space-p3-checkpoint.md) を参照してください。

Native依存に公式 `@capacitor/share@8.0.2` / `@capacitor/filesystem@8.1.3` を追加しています。`npm ci` 後、local自己検証は `npm run mobile:sync:ios`（local-spike）。実機用Staging sync/build/installには別途Human承認と既存の正しいpublic auth設定が必要です。Filesystemのrequired-reasonは `ios/App/App/PrivacyInfo.xcprivacy` のFileTimestamp C617.1、音声bridge payloadを残さないため `loggingBehavior: none` を設定。追加env/schemaはなく、既存0031はStaging適用済み・Production未適用です。今回の未commit実機準備とguardの制限は [Staging/install result](./docs/qss-personal-space-p3-staging-install-result.md) を参照してください。
...
台本専用の限定ブラッシュアップは`NATIVE_MINUTE_ENABLE_SCRIPT_BRUSH_UP=0`が既定です。ONにはlocalで用意した`0037_script_brush_up_foundation.sql`の適用、β quota enforcement、専用のper-user/global/period設定が必要です。保存済みTakeの録音実体と現行revisionを確認し、専用同意と1回の複合quotaで一時voiceと同じ本文の候補音声を作ります。一時voiceの削除確認後だけA/Bとそのrevisionへの採用が可能です。通常のvoiceと3つの既存quotaは変更しません。0037のStaging適用と実provider呼出しはこのlocal foundationには含みません。
