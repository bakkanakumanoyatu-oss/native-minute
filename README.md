...
Gate 4 provider freezeのfresh-user Mobile voice setupは、Bearer-only BFFからexplicit consent、app-owned sample、既存ElevenLabs clone/default voice、Listenへ進む導線まで実装済みです。provider/internal ID、private Storage path、raw provider response、secretはMobile clientへ返しません。actual-deviceのfresh-user clone/cache/provider-unavailable proofは未実施なので、Gate 4はまだcloseしません。詳細は[Gate 4 fresh-user voice readiness result](./docs/g4-provider-production-freeze-fresh-user-voice-readiness-v2-result.md)を参照してください。
