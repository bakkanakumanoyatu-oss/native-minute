...
create temp table deletion_result as select * from public.finalize_account_deletion_database_stage('70000000-0000-4000-8000-000000000030',auth.uid(),'script-revision.account-db.v2');
select pg_temp.assert_true((select db_deleted_row_count=4 and db_retained_row_count=1 and db_observed_row_count=5 from deletion_result),'v2 counts include profile script revisions');
...
select pg_temp.assert_true((select already_finalized from public.finalize_account_deletion_database_stage('70000000-0000-4000-8000-000000000030',auth.uid(),'script-revision.account-db.v2')),'v2 retry');
select 'ACCOUNT_V2_CASCADE_FENCE_COUNTS_PASS';
