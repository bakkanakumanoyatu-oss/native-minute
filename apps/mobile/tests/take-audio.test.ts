...
function fixture(overrides: Record<string, unknown> = {}, missing = false) {
  ...
  return {take,client,download,bucket,calls};
}
...
describe('owned Take playback/share binary boundary',()=>{
 ...
 it('maps unavailable storage to safe retry error and permits a new retry',async()=>{const f=fixture({},true);const r=await handleTakeAudioGet(req(),takeId,deps(f));expect(r.status).toBe(503);expect(await r.text()).not.toContain('private storage');expect((await handleTakeAudioGet(req(),takeId,deps(fixture()))).status).toBe(200)});
