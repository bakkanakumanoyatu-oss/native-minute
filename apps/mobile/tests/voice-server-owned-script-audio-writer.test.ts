...
function configureHappyPath(input: { insertError?: { message: string } | null } = {}) {
  ...
  const insert = vi.fn((values: typeof inserted) => {
    inserted.cache_key = values.cache_key;
    finalAudio.current = inserted;
    return { select: () => ({ single: async () => ({ data: input.insertError ? null : inserted, error: input.insertError ?? null }) }) };
  });
  mocks.createSupabaseAdminClient.mockReturnValue({
    from: vi.fn((table: string) => {
      if (table !== "script_audios") throw new Error(`unexpected admin table: ${table}`);
      return { insert };
    })
  });
  ...
  mocks.stageScriptAudioForReplay.mockResolvedValue({
    storagePath: `/api/script-audio/${AUDIO_ID}`,
    storedAsset: { storageBucket: "script-audios", storageObjectKey: "owned-key", contentType: "audio/mpeg", byteLength: 3 }
  });
  return { client: createClient(finalAudio), insert };
}
...
describe("G5C-B4 server-owned Listen cache writer", () => {
  it("re-authenticates, validates owned script/voice/cache identity, and writes through the server client", async () => {
    const { client, insert } = configureHappyPath();
    ...
    expect(insert).toHaveBeenCalledWith(expect.objectContaining({ script_id: SCRIPT_ID, voice_id: VOICE_ID, provider: "elevenlabs" }));
    expect(mocks.createSupabaseAdminClient).toHaveBeenCalledTimes(1);
  });

  it("maps the database writer fence to a safe product conflict", async () => {
    const { client } = configureHappyPath({ insertError: { message: "voice deletion writer fence is active" } });
