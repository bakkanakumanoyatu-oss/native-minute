...
export type ReviewDisplay = { review: MobileReview; scriptTitle: string; titleConfirmedAt?: number };
...
export function reviewDisplayMemory(api: Pick<PracticeApi, "getReview" | "getScript" | "savedTakeAudioMemory">, ownerIsCurrent?: () => boolean, events: Events<ReviewDisplay> = {}) {
  return new DisplayMemory<ReviewDisplay>(async (key, { previous, signal, reason }) => {
    ...
    const titleNeeded = previous?.titleConfirmedAt === undefined || performance.now() - previous.titleConfirmedAt >= METADATA_REFRESH_AGE_MS || reason === "manual";
    ...
    return { kind: "success", data: { review: result.review, scriptTitle: script?.kind === "success" ? script.script.title : previous!.scriptTitle,
