...
function fixture() {
  ...
  let token = "token-a", version: string | null = identity, status = 200;
  const fetchImpl = vi.fn(async (url: string | URL | Request) => {
    if (String(url).endsWith("/audio")) return new Response(new Blob(["owned audio"], { type: "audio/wav" }), { headers: { "content-type": "audio/wav", "content-disposition": "attachment; filename*=UTF-8''Take.wav", "x-take-audio-identity": version ?? "" } });
  ...
  return { api, enter, counts, setStatus: (v: number) => { status = v; }, setVersion: (v: string | null) => { version = v; }, switchSession: () => { token = "token-b"; }, switchOwner: () => { state = { kind: "authenticated", userId: "other" }; } };
}
