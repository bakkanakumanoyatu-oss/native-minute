...
import { saveModelAudioRequestSchema } from "@/schemas/audio-library";
import { AudioLibraryError, listSavedModelAudios, saveModelAudio } from "@/services/audio-library";
...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const savedModelAudios = await listSavedModelAudios(supabase, user.id, parsedScriptId.data);
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const supabase = createSupabaseRouteClient();
    const user = await requireCurrentUser(supabase);
    const savedModelAudio = await saveModelAudio(supabase, user.id, {
      scriptId: parsedScriptId.data,
      scriptAudioId: parsedBody.data.scriptAudioId,
