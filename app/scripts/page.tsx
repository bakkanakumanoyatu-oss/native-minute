...
export default async function ScriptsPage() {
  ...
  return (
    <section className="space-y-6">
      ...
      {!voiceReady ? (
        <div className="rounded-[2rem] border border-[var(--line-inset)] bg-[var(--coach-note)] px-5 py-4 text-sm leading-6 text-ink-700">
      ...
      {scripts.length === 0 ? (
        <div className="rounded-[2rem] border border-[var(--line-inset)] bg-[linear-gradient(180deg,var(--studio-surface-secondary),var(--studio-surface-inset))] p-6 shadow-[var(--shadow-studio-soft)]">
          <h2 className="text-2xl font-semibold text-ink-900">最初の1分を作りましょう。</h2>
          <p className="mt-3 rounded-2xl border border-[var(--line-subtle)] bg-[var(--script-paper)] px-4 py-3 text-sm leading-6 text-ink-700">映画やドラマのセリフっぽい言い回し、仕事、旅行、自己紹介など、話してみたいテーマから始められます。</p>
          <Link href="/scripts/new" className="mt-5 inline-flex rounded-2xl bg-[var(--cta-primary-bg)] px-5 py-3 text-sm font-semibold text-[var(--cta-primary-text)]">
            1分を作る
          </Link>
        </div>
      ) : (
        <div className="grid gap-5">
          {visibleScripts.map((item) => {
            ...
            return (
              <article key={item.script.id} className="overflow-hidden rounded-[2rem] border border-[var(--line-inset)] bg-[linear-gradient(180deg,var(--studio-surface-secondary),var(--studio-surface-inset))] p-6 transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-studio-soft)] sm:p-7">
                <div aria-hidden="true" className="mb-5 h-1.5 rounded-full bg-[rgba(24,23,34,0.12)]" />
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_15rem] lg:items-start">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full border border-[var(--line-subtle)] bg-[var(--take-paper)] px-3 py-1 text-xs font-semibold text-ink-600">
                        {item.takeCount > 0 ? `Take ${item.takeCount}回` : "まだ録っていない"}
                      </span>
                    </div>
                    <p className="mt-4 rounded-2xl border border-[var(--line-subtle)] bg-[var(--script-paper)] px-4 py-3 text-base leading-7 text-ink-700">{getFirstLine(item.script.content)}</p>
                  </div>
                  <div className="rounded-[1.5rem] border border-[var(--line-inset)] bg-[var(--coach-note)] p-5">
                ...
                <details className="mt-4 rounded-2xl border border-[var(--line-subtle)] bg-[rgba(223,197,170,0.58)] px-4 py-3">
                  ...
                  <div className="mt-4 grid gap-2 text-sm font-semibold sm:max-w-xs">
                    <Link href={getScriptListenPath(item.script.id)} className="rounded-2xl border border-[var(--line-inset)] bg-[var(--surface-inset)] px-4 py-3 text-ink-700">
                      お手本へ
                    </Link>
                    <Link href={getScriptRecordPath(item.script.id)} className="rounded-2xl border border-[var(--line-inset)] bg-[var(--surface-inset)] px-4 py-3 text-ink-700">
                      録る
                    </Link>
