...
- `/setup/voice` now treats re-recording as a normal v1 setup action: users with an existing default voice first choose whether to keep the current voice, record a new sample, or use an existing audio file. The recording / file input area opens only after that choice. This reuses the existing consent / sample upload / createVoice path and does not perform provider cleanup or deletion.
- Voice setup rerecord choices production human smoke is `PASS` on iPhone Safari. The existing-default-voice state shows a clear `次に使う声を選ぶ` heading and the three choices, does not over-expose the recording form initially, keeps the no-old-voice-deletion explanation visible, and did not expose raw audio, storage paths, tokens, or secrets.
- Voice setup rerecord practical E2E production human smoke is also `PASS` on iPhone Safari. From an existing default voice, selecting `新しく録音して作り直す` completed in-browser recording, stop, playback confirmation, and creating a new model voice without the prior iPhone recording format error. No old voice deletion or provider cleanup was run.
- Store release short refresh after the voice setup PASS: Web core, auth callback, legal / support / account deletion public flow, mobile browser QA, five-slot practice management, voice setup rerecord choices, and practical voice setup rerecord E2E are complete enough to return to screenshot preparation. Remaining Store-submission work is screenshot / Store assets, reviewer account final login, Android / Google Play checks, Capacitor/native packaging, actual account deletion proof, provider production readiness / cleanup proof, App Privacy / Data Safety final answers, and final release QA.
- Screenshot work should resume with the minimum set Home / Practice entry, Script creation, Listen, Record, Review, and Progress. Voice setup three-choice state, Settings, legal, and account deletion request surfaces are optional support/trust screenshots only if final Store slots and redaction review allow.
- Screenshot candidate selection review is captured in `outputs/store_screenshot_candidate_selection_review/`. Human-reviewed strong candidates: Home / Practice entry, Script creation, Listen / model voice, Review, and Voice setup three-choice state. Lower-priority holds: voice log shelf, playback-speed-only view, and best-take plus replay-speed view. Missing / retake candidates: Record in normal ready/evaluate state and Progress showing history / growth / recent or best takes. Current Safari captures are review candidates only; final Store assets should avoid Safari UI, URL bars, browser controls, loading states, private identifiers, raw audio / storage paths, secrets, and unimplemented v1.1 claims.
- Record / Progress screenshot candidate review update is captured in `outputs/store_screenshot_record_progress_review/`. Record should use the English script + Take recording + model voice playback view, not the detailed take file view that exposes recording filename, MIME / codec, size, duration, or too-short warning. Progress should use the latest take + best take view because it communicates history, reflection, score, memo, replay, and best-take value; the voice log shelf / one-minute stock view remains lower priority. The updated final set is Home / Practice entry, Script creation, Listen, Record, Review, Progress, and Voice setup three-choice state.
- Final screenshot set redaction review / retake preparation is captured in `outputs/store_final_screenshot_redaction_review/`. The seven-screen composition is effectively set: Home / Practice entry, Script creation, Listen / model voice, Record, Review, Progress, and Voice setup three-choice state. Existing Safari captures remain candidates only, not final submission assets. Final retakes should be app-display or Store-ready captures without Safari UI, URL bars, browser controls, account identifiers, raw audio or Storage paths, secrets, magic links, recording filenames, MIME / codec / file-size / storage-object metadata, provider names or internal IDs, error / loading / network-failure states, negative short-recording warnings where avoidable, unimplemented v1.1 claims, actual-deletion-complete wording, or visibly cut-off text.
- Final screenshot capture strategy is captured in `outputs/store_final_screenshot_capture_strategy/`. Current Web / Safari screenshots remain composition references only. Final Store submission assets should be captured after Capacitor in app display, with no Safari UI / URL bars / browser controls. The seven-screen set remains Home / Practice entry, Script creation, Listen, Record, Review, Progress, and Voice setup three-choice state. Before final capture, prepare a clean demo account, use normal non-error states, prefer non-negative Review / Progress content, keep Record away from recording-file metadata, use the existing-voice three-choice setup state, and run a final redaction pass after capture. This is strategy-only; no code, UI, screenshot capture, image editing, Store Console work, Capacitor, provider, schema, or actual deletion work is performed.
- Final app-display screenshot procedure is captured in `outputs/store_final_screenshot_capture_procedure/`. It fixes the seven capture targets, desired app states, demo-account prerequisites, redaction rules, and Capacitor-specific capture risks before any implementation. Required demo state: a clean account with at least one safe script, reusable model audio, a normal Record path, at least one reviewable Take, latest / best take data visible in Progress, and an existing default voice so `/setup/voice` shows the three-choice state. Capture must wait for Capacitor app-display preflight and should check safe area, viewport, status bar, bottom navigation, microphone permission, and audio playback permission before final Store screenshots. No code, UI, Capacitor, screenshot capture, image editing, Store Console, provider, schema, or actual deletion work is performed.
- Capacitor preflight checklist is captured in `docs/capacitor-preflight-checklist.md` and `outputs/capacitor_preflight_checklist/`. The current Next.js app depends on App Router server rendering, API routes, Supabase cookie auth, protected replay routes, upload routes, and server-side provider/storage boundaries, so the first native direction should be treated as hosted WebView preflight rather than static export. Main preflight areas: package/build scripts, API/server-only dependencies, magic link callback and future deep link needs, mic/audio permissions, upload MIME and ownership, safe area / viewport / status bar / bottom navigation, legal/support/deletion links inside app display, screenshot impact, provider readiness boundaries, and Android / Google Play unknowns. No package install, Capacitor init, iOS/Android project, code, UI, Store Console, provider, schema, or actual deletion work is performed.
- Capacitor introduction decision is captured in `docs/capacitor-introduction-decision.md` and `outputs/capacitor_introduction_decision/`. Decision: proceed to a later Capacitor install gate with hosted WebView first, not static export; keep API routes, server logic, provider secrets, and API keys on Vercel/server; start with `ios_only_preflight`; do not require complete native magic-link deep link support before the first smoke; keep custom scheme / universal links as a later auth gate. First native smoke scope is `standard`: app launch, production URL load, Home, bottom navigation, Listen playback, Record microphone prompt and start/stop, Review, Progress, Voice setup three-choice state, and privacy / support / account deletion links. Human decisions still needed: iOS-first approval, Android device timing, reviewer account final login sequencing, and provider production readiness sequencing. No package install, Capacitor init, iOS/Android project, code, UI, Store Console, provider, schema, or actual deletion work is performed.
- Capacitor introduction guardrail update is captured in `outputs/capacitor_introduction_guardrail_update/` and reflected in `docs/capacitor-introduction-decision.md`. Hosted WebView first is now explicitly `preflight_only`: using Capacitor `server.url` may be acceptable for the first native shell / app-display / permission / WebView cookie smoke, but it is not a Store-submission-ready architecture claim. Final Store architecture requires a follow-up gate before release. `proceed_to_capacitor_install` is updated to `conditional_true_after_bundle_id_approval`, and appId / Bundle ID must receive human approval before `capacitor init`; no real identifier is fixed here. iOS-only start is approved, Android waits until after iOS native smoke, reviewer account final login and provider readiness are not blockers before first Capacitor install, and provider readiness remains a blocker before release QA. No package install, Capacitor init, iOS/Android project, code, UI, Store Console, provider, schema, or actual deletion work is performed.
- Capacitor appId / Bundle ID approval is captured in `outputs/capacitor_bundle_id_approval/` and reflected in `docs/capacitor-introduction-decision.md`. The human-approved appId / Bundle ID is `com.nativeminutes.app`. This satisfies the pre-init identifier approval guardrail, but it does not perform package install, `capacitor init`, bundle registration, Apple Developer / Store Console work, or iOS/Android project creation. Sequencing remains: iOS-only preflight first, Android after iOS native smoke, hosted WebView / `server.url` preflight-only, final Store architecture follow-up before submission, reviewer account final login after first native smoke, provider readiness after first native smoke but before final release QA, and provider secrets / API keys / service-role keys stay out of the native client.
- Capacitor iOS preflight shell is installed in `outputs/capacitor_ios_preflight_install/`. Added `@capacitor/core`, `@capacitor/cli`, and `@capacitor/ios` 8.4.0, created `capacitor.config.ts`, added a minimal `www/index.html` placeholder, and generated the iOS project under `ios/` with appId / Bundle ID `com.nativeminutes.app` and app name `Native Minutes`. The config uses `https://native-minute.vercel.app` as preflight-only `server.url`; this does not claim Store submission readiness and final Store architecture still requires a later gate. `NSMicrophoneUsageDescription` is present for iOS Record permission smoke. Android, Xcode/device run, Store Console, deep links / universal links, auth callback changes, provider connection, DB schema, actual deletion, and final screenshots are not performed.
- Capacitor iOS native smoke checklist is captured in `docs/capacitor-ios-native-smoke-checklist.md` and `outputs/capacitor_ios_native_smoke_checklist/`. It tells the human to open `ios/App/App.xcworkspace` in Xcode, prefer an iPhone device when available, and record `PASS / FAIL / BLOCKED / NOT_CHECKED` for launch, production URL, Home, bottom navigation, safe area / status bar / keyboard, auth/session, Listen playback, Record mic permission and start/stop, voice setup, Review, Progress, legal links, and screenshot readiness. Android remains out of scope, `server.url` stays preflight-only, deep links / universal links are not first-smoke blockers, and actual deletion / provider connection / final screenshot capture are not performed.
- Initial iOS native smoke result: shell launch, iPhone 16 Pro Simulator launch, and Native Minutes Home render are `PASS`. Protected routes showing the login prompt while unauthenticated are treated as expected route-guard behavior, not the main blocker. Auth/session is `BLOCKED` because the magic link callback opened in Mac Chrome rather than the iOS WebView, so the callback browser context lacked the PKCE verifier cookie and landed on `/login?error=callback_pkce_missing&next=%2Fscripts`. Next work should be a native auth gate that chooses between native deep link / universal link support, a native-specific login path, or a TestFlight-before-release auth callback gate. Do not treat `server.url` as Store-submission-ready architecture, and do not change DB schema, providers, ownership, or auth security boundaries as part of this status note.
- Native auth gate decision is captured in `docs/capacitor-native-auth-gate-decision.md`. Recommendation: close the v1 native auth blocker with universal link / deep link callback return to the iOS app and WebView, preserving the existing `/auth/callback` PKCE and server-cookie flow. Native OTP / email-code login is a fallback option with broader auth/UI surface, while keeping auth blocked only defers a release blocker. No deep link, universal link, native login, Apple Developer, Supabase production setting, DB schema, provider, or Store-ready architecture work is performed in this decision packet.
- Universal Link primary preimplementation checklist is captured in `docs/capacitor-universal-link-preimplementation-checklist.md`. It defines the Apple Associated Domains, AASA, Supabase redirect allowlist, repo-side AASA delivery, iOS entitlement, AppDelegate URL handling, custom scheme fallback, smoke procedure, and stop-condition checklist before implementation. No Universal Link implementation, entitlement, AASA file, Supabase setting change, Apple Developer work, AppDelegate implementation, DB schema, provider, or Store-ready architecture change is performed.
