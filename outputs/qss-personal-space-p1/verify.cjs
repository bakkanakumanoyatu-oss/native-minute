...
(async()=>{
 ...
 for (const width of [320,428]) for(const textSize of [16,32]) {
  ...
  async function check(name, path, heading) {
   await page.goto('http://127.0.0.1:5191'+path);
   ...
   await page.screenshot({path:`outputs/qss-personal-space-p1/${name}-${width}-${textSize}.png`,fullPage:true});
  ...
  await expect(page.getByRole('button',{name:/Favorite|お気に入り|Share|名前/})).toHaveCount(0);
 ...
 await browser.close();fs.writeFileSync('outputs/qss-personal-space-p1/verification.json',JSON.stringify(results,null,2));console.log(results.length+' checks PASS');
