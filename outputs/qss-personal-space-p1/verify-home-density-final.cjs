...
(async () => {
  ...
  for (const width of [320, 428]) for (const fontSize of [16, 32]) {
    ...
    for (const fixture of ['first', 'no-result', 'empty', 'first-long', 'one', 'populated', 'long', 'error']) {
      await page.goto(`http://127.0.0.1:5191/?fixture=${fixture}`);
      ...
      if (fixture === 'error') {
        ...
        await expect(page.getByRole('button', { name: /お気に入り|Favorite|名前|Share/ })).toHaveCount(0);
      ...
      await expect(page.locator('.home-screen')).not.toContainText('お気に入り');
      await expect(page.locator('.home-screen')).not.toContainText('準備中');
      expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
      await page.screenshot({ path: `outputs/qss-personal-space-p1/density-final-${fixture}-${width}-${fontSize}.png`, fullPage: true });
      results.push({ fixture, width, fontSize, status: 'PASS' });
    }
    await page.goto('http://127.0.0.1:5191/takes?fixture=long');
    await expect(page.getByRole('heading', { name: '録音履歴' })).toBeVisible();
    await page.evaluate(size => document.documentElement.style.fontSize = size + 'px', fontSize);
    ...
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
    await page.screenshot({ path: `outputs/qss-personal-space-p1/density-final-takes-long-${width}-${fontSize}.png`, fullPage: true });
    results.push({ fixture: 'takes-long-title', width, fontSize, status: 'PASS' });
    await page.goto('http://127.0.0.1:5191/?fixture=first');
    ...
    await page.goto('http://127.0.0.1:5191/?fixture=populated');
  ...
  await browser.close();
  fs.writeFileSync('outputs/qss-personal-space-p1/verification-home-density-final.json', JSON.stringify(results, null, 2));
