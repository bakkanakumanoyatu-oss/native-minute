...
(async()=>{
 ...
 for(const width of [320,428])for(const textSize of [16,32]){
  ...
  for(const screen of ['listen','record','review']){
   ...
   await page.goto('http://127.0.0.1:5198/'+(screen==='review'?'takes':'scripts')+'?fixture=one');
   ...
   await page.screenshot({path:`outputs/qss-personal-space-p3/exit-home-${screen}-${width}-${textSize}.png`});
   await exit().click();
   ...
   await page.goto('http://127.0.0.1:5198/scripts/preview-script/'+path);
   await expect(page.locator('.'+screen+'-screen')).toBeVisible();
   await back().click();
   await expect(page).toHaveURL(screen==='review'?/\/record$/:screen==='record'?/\/listen$/:/5198\/$/);
 ...
 await browser.close();fs.writeFileSync('outputs/qss-personal-space-p3/exit-home-results.json',JSON.stringify(results,null,2));console.log(results.length+' direct practice exit regressions PASS');
