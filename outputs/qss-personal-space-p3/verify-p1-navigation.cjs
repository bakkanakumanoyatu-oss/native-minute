...
(async()=>{
 ...
 for (const width of [320,428]) for(const textSize of [16,32]) {
  ...
  async function check(name, path, heading) {
   ...
   await page.screenshot({path:`outputs/qss-personal-space-p3/${name}-${width}-${textSize}.png`,fullPage:true});
  ...
  await check('review','/scripts/preview-script/review/preview-latest','A small pause');
 ...
 await browser.close();fs.writeFileSync('outputs/qss-personal-space-p3/verification.json',JSON.stringify(results,null,2));console.log(results.length+' checks PASS');
