...
(async()=>{
 ...
 for(const width of [320,428])for(const size of [16,32]){
  ...
  await page.goto('http://127.0.0.1:5198/scripts/preview-script/review/preview-latest?fixture=one');
  ...
  await expect(page.locator('.take-identity h1')).toHaveText('A small pause');
  ...
  await page.goto('http://127.0.0.1:5198/takes?fixture=error&filter=favorites');
 ...
 await browser.close();fs.writeFileSync('outputs/qss-personal-space-p3/error-results.json',JSON.stringify(results,null,2));console.log(results.length+' error conditions PASS');
