import { TakeShareController, takeShareController } from "../../apps/mobile/src/audio/take-share";
import { takeExportFilename } from "../../lib/take-audio-format";
...
const mode = new URLSearchParams(location.search).get("fixture") ?? sessionStorage.getItem("qss-p3-fixture") ?? "populated";
sessionStorage.setItem("qss-p3-fixture", mode);
...
const storageKey = 'qss-p3-metadata-' + mode;
...
const wav = encodeMonoPcm16Wav(new Float32Array(48000).map((_,i) => Math.sin(i * 0.06)*0.15), 24000);
window.p3QA = { audioMode: 'success', downloads: [], files: [], exported: [], outcome: 'finish', pending: null, maxFiles: 0, snapshot: () => JSON.stringify(selected) };
const fakeNativeController = new TakeShareController({
 supported: () => true,
 cleanup: async () => { window.p3QA.files = []; },
 write: async (name, data) => { window.p3QA.files.push(name); window.p3QA.maxFiles = Math.max(window.p3QA.maxFiles,window.p3QA.files.length); return 'file:///synthetic-cache/' + name; },
 share: async uri => {
  window.p3QA.exported.push(uri);
  if (window.p3QA.outcome === 'hold') await new Promise(resolve => window.p3QA.pending = resolve);
  if (window.p3QA.outcome === 'cancel') throw new Error('Share canceled');
  if (window.p3QA.outcome === 'fail') throw new Error('synthetic native failure');
 }
});
takeShareController.supported = () => true;
takeShareController.share = fakeNativeController.share.bind(fakeNativeController);
...
const api = new Proxy({
  downloadTakeAudio: async takeId => {
    window.p3QA.downloads.push(takeId);
    if (window.p3QA.audioDelay) await new Promise(resolve => setTimeout(resolve, window.p3QA.audioDelay));
    if (window.p3QA.audioMode !== 'success') return window.p3QA.audioMode === 'missing' ? { kind: 'not-found' } : { kind: 'network-error' };
    const row = selected.find(row => row.takeHistory.some(take => take.id === takeId));
    const found = row?.takeHistory.find(take => take.id === takeId);
    if (!found) return { kind: 'not-found' };
    return { kind: 'success', audio: new Blob([wav], {type:'audio/wav'}), contentType: 'audio/wav', filename: takeExportFilename(found.displayName, row.script.title, 'wav') };
  },
...
createRoot(document.getElementById("root")).render(<>
  <aside style={{ position: "fixed", top: 0, left: 0, zIndex: 50, background: "#eaf0f2", fontSize: 10, padding: 4 }}>
    P3 QA fixture (native adapter fake)・合成データ｜<a href="/?fixture=first">初回</a> / <a href="/?fixture=populated">複数</a> / <a href="/?fixture=one">1件</a> / <a href="/?fixture=no-result">結果なし</a> / <a href="/?fixture=long">長い台本名</a> / <a href="/?fixture=error">失敗</a>
  </aside>
