...
export function ScriptPracticeChunks({
  ...
  return (
    <section data-testid={testId} className="rounded-[2rem] border border-[var(--line)] bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <p className="rounded-2xl border border-[var(--line)] bg-ink-50 px-4 py-3 text-sm leading-6 text-ink-700">{actionCue}</p>
      </div>

      {limitedFocusWords.length > 0 ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {limitedFocusWords.map((word) => (
            <span key={word} className="rounded-full border border-[var(--line)] bg-ink-50 px-3 py-1 text-xs font-semibold text-ink-700">
              focus: {word}
            </span>
          ))}
        </div>
          ...
          return (
            <li key={`${chunk.index}-${chunk.text}`} className="rounded-2xl border border-[var(--line)] bg-ink-50 px-4 py-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                ...
                <div className="shrink-0 rounded-full bg-white px-3 py-1 text-xs font-semibold text-ink-700">
